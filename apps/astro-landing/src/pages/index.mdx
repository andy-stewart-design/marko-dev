...
Importing `.marko` files in `.astro` or `.ts` files is **loosely** typed automatically — the integration injects an ambient `*.marko` module declaration into your project via Astro's automated type generation. Full per-component `Input` type inference (typed props) is not yet supported and is still being explored.
